...
headers = "County,State,FIPS,Latitude,Longitude,Cases,Deaths\n"
...
if (tables[1].find('td').text) == 'Adair' and (tables[134].find('td').text) == 'TBD':
    ...
    for t in tables[1:92]:
        ...
        file.write(pull[0].text + ", " + mo + ", " + fips.get_county_fips(pull[0].text, state=mo) + ", " + str(locale.latlng).strip('[]')
                    + ", " + pull[1].text + "\n")
        
    file.write(tables[92].findAll('td')[0].text + ", " + mo + ", " + fips.get_state_fips(mo) + ", " + str(geocoder.opencage(mo, key='').latlng).strip('[]')
    ...
    file.write(tables[134].findAll('td')[0].text + ", " + mo + ", " + fips.get_state_fips(mo) + ", " + str(geocoder.opencage(mo, key='').latlng).strip('[]')
